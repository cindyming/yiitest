...
class GlobaltotalController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $zhuijianvertTotal = $connection->createCommand("SELECT sum(amount) as 'total' FROM investment LEFT JOIN user on user.id=investment.user_id WHERE  investment.created_at >'{$date}' AND user.approved_at<'{$date}' AND investment.status=1")->queryOne();
        ...
        $bonus  = $connection->createCommand("SELECT sum(merit) as 'merit_total', sum(baodan) as 'baodan_total'  FROM revenue WHERE created_at > '{$date}' AND type=1")->queryOne();
