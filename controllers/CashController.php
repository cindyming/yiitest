                        ...
                        try {
                            ...
                            if (($model->cash_type == 3) && !System::loadConfig('duichong_audit')) {
                                ...
                                if (($model->cash_type == 4) && !System::loadConfig('mall_audit')) {
                                    ...
                                    $data = array('account' => $model->sc_account, 'money' => floatval($realAmount), 'accountType' => '海贝', 'system' => '昆明地产' . $model->user_id, 'mobile' => $model->telephone);
                                    if ($model->type != 9) {
                                        $data['accountType'] = '海币';
                                    }
                                    ksort($data);
        ...
        if ($model->status == 1) {
            try {
                ...
                if ($model->cash_type == 1) {
                    ...
                    Log::add('会员(' . $model->user_id . ')', '提现股票转移', '返回', json_encode($curl_response));
                    ...
                    $data = array('account' => $model->sc_account, 'money' => floatval($model->real_amount), 'accountType' => '海贝', 'system' => '昆明地产' . $model->user_id, 'mobile' => $model->telephone);
                    if ($model->type != 9) {
                        $data['accountType'] = '海币';
                    }
                    ksort($data);
