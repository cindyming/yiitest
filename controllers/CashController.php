                        ...
                        try {
                            ...
                            if (($model->cash_type == 3) && !System::loadConfig('duichong_audit')) {
                                ...
                                if (($model->cash_type == 4) && !System::loadConfig('mall_audit')) {
                                    ...
                                    $data = array('account' => $model->sc_account, 'money' => floatval($realAmount), 'accountType' => '海币', 'system' => '玫瑰家园' . $user->username, 'mobile' => $model->telephone);
                                    if ($model->type != 9) {
                                        $data['accountType'] = '海宝';
                                    }
                                    ksort($data);
    ...
    public function actionAdminapprove($id)
    {
        ...
        try {
            ...
            if ($model->cash_type == 1) {
                ...
                Log::add('会员(' . $model->user_id . ')' , '提现股票转移' , '返回' , $curl_response);
                ...
                $data = array('account' => $model->sc_account, 'money' => floatval($model->real_amount), 'accountType' => '海币', 'system' => '玫瑰家园' . $user->username, 'mobile' => $model->telephone);
                if ($model->type != 9) {
                    $data['accountType'] = '海宝';
                }
                ksort($data);
        ...
        if (Yii::$app->request->post() && $model->load($data)) {
            ...
            if ($user && $user->id) {
                ...
                if(in_array($model->type, array(4,5,6,7, 8))) {
                    ...
                    if ($model->amount > $compareAmount) {
                        $validateAmount = false;
                        $model->addError('amount', '可供提现余额不足, 请确认后重新输入. 分红余额: ' . $user->bonus_remain . ', 绩效余额: ' . $user->merit_remain .  ', 服务费余额: ' . (float)$user->baodan_remain  .  ', 商城币余额: ' . (float)$user->mall_remain  . '.');
                    }
                } else {
                    $validateAmount = false;
                    $model->addError('type', '请选择账户类型.');
