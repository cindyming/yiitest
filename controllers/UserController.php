        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['adminindex', 'export', 'cancel', 'huobi','validate', 'admincreate', 'suggestindex', 'adminreject','success','adminresetpassword','adminapplyindex','adminchange','adminapproveforaddmember', 'admintree', 'admintreelazy', 'adminindexapprove', 'adminindexunapprove', 'adminupdate', 'adminview', 'adminapprove'],
        ...
        foreach($users as $user) {
            if ($user['id'] == Yii::$app->user->identity->id) {
                ...
                $result[] = array(
                    "id" => $user['id'],
                    "parent" => '#',
                    "text" => $user['id']. "(昵称: " . $user['username']  . ", 投资额 : " . ($user['investment'] / 10000) . "万,".(Yii::$app->user->identity->showTotal()? " 总业绩 : "  . ($user['achievements']/10000) . "万)" : ")")
                );
                ...
                if ($id == $user['id']) {
                    $result[] = array(
                        ...
                        "text" => $user['id']. "(昵称: " . $user['username']  . ", 投资额 : " . ($user['investment'] / 10000) . "万,".(Yii::$app->user->identity->showTotal() ? " 总业绩 : "  . ($user['achievements']/10000) . "万)" : ")") . (($user['role_id'] == 2) ? ' - 待审核' : ''),
                        "state" => array(
                            "opened" => true,
                            "selected" => true
                        )
                    ...
                    $result[] = array(
                        ...
                        "text" => $user['id'] . "(昵称: " . $user['username']  . ", 投资额 : " . ($user['investment'] / 10000) . "万,".(Yii::$app->user->identity->showTotal() ? " 总业绩 : "  . ($user['achievements']/10000) . "万)" : ")") . (($user['role_id'] == 2) ? ' - 待审核' : ($user['locked'] ? ' - 已锁定' : ''))
                    );
                }
                $ids[] = $user['id'];
            }
 ...
 */
    ...
    public function actionExport()
    {
        ...
        if (Yii::$app->request->get('week', 0)) {
            $data['UserSearch']['approved_at'] = date('Y-m-d', strtotime('-7 days')) . ' - ' .date('Y-m-d', time());
        } else if ((!isset($data["UserSearch"])) && (!isset($data["UserSearch"]['approved_at']))) {
            $data['UserSearch']['approved_at'] = date('Y-m-d', strtotime('-7 days')) . ' - ' .date('Y-m-d', time());
        }
        $searchModel->export($data);
        return $this->redirect(['/assets/Users.xls']);
    }
}
