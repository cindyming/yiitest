...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['qq', 'useBaodan', 'init_investment', 'stack', 'init_stack', 'redeemed', 'total_stack', 'free_stack', 'total_free_stack', 'total_investment', 'exchange_total', 'exchange_remain', 'be_stack'], 'number'],
    ...
    */
    ...
    public function afterSave($insert, $changedAttributes)
    {
        if ($insert) {
            ...
            $service_url = Yii::$app->params['cuohe_url'] . 'api/user/create';
            $curl = curl_init($service_url);
            curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
            curl_setopt($curl, CURLOPT_USERPWD, Yii::$app->user->identity->access_token); //Your credentials goes here
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_POST, true);
            ...
            $response = (array)json_decode($curl_response);
            curl_close($curl);
            Log::add('会员(' . $this->id . ')' , '撮合创建' , '返回' , $service_url . ':' . $curl_response);
        }
...
2016年8月1日至9月16日早0点的奖励10%（规则同上）
        ...
        $mappings = array(
            ...
            array('from' => 20170603, 'to' => 20170831, 'price' => 2.45),
